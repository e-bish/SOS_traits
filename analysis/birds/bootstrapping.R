...
# boot_list <- lapply(1:20, function(i) mat_to_boot[sample(nrow(mat_to_boot)),])
...
for (i in 1:length(boot_L)){
  ...
  # for (j in 1:length(FD_indices)){
  #   results_list[[j]] <- rbind(results_list[[j]], FD_values_df[,c(1,j+1)])
  # }
}
#the mFD package uses ape::pcoa() which automatically removes negative eigenvalues rather than applying a correction

# FD_results_df <- map_df(results_list, ~as.data.frame(.x), .id="id")

# names(FD_results[2:6]) <- c("Species_Richness", "FDis", "FEve", "FRic", "FDiv")
library(ggridges)
